...
function NET.player_publishStats(S)
    NET.player_setState(JSON.encode{
        techminoStats={
            piece=tonumber(S.piece) or 0,
            time=tonumber(S.time) or 0,
            atk=tonumber(S.atk) or 0,
            dig=tonumber(S.dig) or 0,
        },
    })
end
...
function NET.wsCallBack.player_setState(body)
    if SCN.cur~='net_game' or type(body.data)~='table' then return end
    local state=type(body.data.customState)=='string' and JSON.decode(body.data.customState)
    if type(state)=='table' and type(state.techminoStats)=='table' then
        NETPLY.setStat(body.data.playerId,state.techminoStats)
    end
end

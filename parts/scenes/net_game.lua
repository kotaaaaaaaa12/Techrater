    ...
    if playing then
        if not TASK.getLock('netPlaying') then
            ...
            if #PLAYERS>0 then
                ...
                then
                    NET.player_publishStats(P1.stat)
                    NET.player_finish({reason='lose'})
                end

                if not NET.spectate and P1.frameRun-lastStatUpstreamTime>=60 then
                    NET.player_publishStats(P1.stat)
                    lastStatUpstreamTime=P1.frameRun
                end

...
function NETPLY.setStat(uid,S)
    local time=tonumber(S and S.time)
    if not time or time<=0 then return end
    local piece=tonumber(S.piece) or 0
    local atk=tonumber(S.atk) or 0
    local dig=tonumber(S.dig) or 0
    PLYmap[uid].stat={
        lpm=("%.1f %s"):format(piece/time*24,text.radarData[5]),
        apm=("%.1f %s"):format(atk/time*60,text.radarData[3]),
        adpm=("%.1f %s"):format((atk+dig)/time*60,text.radarData[2]),
    }
end
